...
void insertionsortSLL(struct node** head)
{
    ...
    while(j!= NULL && i->value < j->value)
    {
           int t = j->value;
            j->value =i->value;
           i->value = t;
// while working on array we push j to j+1 until the smaller i becomes smaller hence we dont exchanges values
// we assign them j to j+1 as we know at the last place we will update i , but here in LL we cant do that so we have token
// swap the j and j+1 and update i basically the key elemt we would have switched later.
            i = j;   
            j = j->prev; 
    }
